...
const LocationPicker: FC<Props> = ({ initialLatitude = 50.62, initialLongitude = 26.25, initialAddress = '', onResolve }) => {
  ...
  const [mapTilerEnabled, setMapTilerEnabled] = useState(Boolean(key));
  ...
  const mapStyle = useMemo(() => mapTilerEnabled ? `https://api.maptiler.com/maps/basic-v2/style.json?key=${key}` : ({ version: 8, sources: { osm: { type: 'raster', tiles: ['https://a.tile.openstreetmap.org/{z}/{x}/{y}.png'], tileSize: 256 } }, layers: [{ id: 'osm', type: 'raster', source: 'osm' }] } as StyleSpecification), [key, mapTilerEnabled]);
  ...
  const search = async () => {
    ...
    if (!mapTilerEnabled) { setMessage('Move the map, confirm the pin, then complete the street and building fields below.'); return; }
  ...
  const confirmPin = async () => {
    if (!mapTilerEnabled) { onResolve({ formattedAddress: query.trim(), street: query.trim(), building: '', latitude: viewState.latitude, longitude: viewState.longitude }); setMessage('Map location confirmed. Complete the street and building fields below.'); return; }
