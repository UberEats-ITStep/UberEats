...
const Profile: FC = () => {
  ...
  const saveProfile = async (event: React.FormEvent) => { event.preventDefault(); setSavingProfile(true); setNotice(null); try { const normalizedPhone = phone.trim(); await authApi.updateProfile({ avatar, ...(normalizedPhone ? { phone_number: normalizedPhone } : {}) }); await refreshProfile(); setNotice({ kind: 'success', message: 'Profile updated.' }); } catch (err: unknown) { const response = (err as { response?: { data?: { phone_number?: string[] } } }).response; setNotice({ kind: 'error', message: response?.data?.phone_number?.[0] || 'Profile could not be updated.' }); } finally { setSavingProfile(false); } };
